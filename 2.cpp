...
void DrawGraphAndSolve(Line *lines, float **Table)
{
	...
	pointA = make_pair(0, maxnum(lines[0].y2, lines[1].y2, lines[2].y2));
	pointD = make_pair(maxnum(lines[0].x1, lines[1].x1, lines[2].x1), 0);
	...
	*/
	...
	if (line1b + line2b > line1c + line2c)
	{
		cout << " Первая точка входа: С" << endl;
		solutionline = 1;
	}
	else
	{
		cout << " Первая точка входа: B" << endl;
		solutionline = 2;
	}
